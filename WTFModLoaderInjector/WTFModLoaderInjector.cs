...
namespace WTFModloaderInjector
{
	internal static class WTFModLoaderInjector
    {
        ...
        private const string MOD_LOADER_DLL_FILE_NAME = "WTFModLoader.dll";
        private const string GAME_DLL_FILE_NAME = "WTF.exe";
        ...
        private static readonly OptionSet Options = new OptionSet
        {
            {
                "d|detect",
                "Detect if the WTF game assembly is already injected",
                v => OptionsIn.Detecting = v != null
            },
            {
                "g|gameversion",
                "Print the WTF game version number",
            ...
            {
                "manageddir=",
                "specify managed dir where WTF game's Assembly-CSharp.dll is located",
            ...
            {
                "requiredversion=",
                "Don't continue with /install, /update, etc. if the WTF game version does not match given argument",
                v => OptionsIn.RequiredGameVersion = v
            },
            {
                "r|restore",
                "Restore pristine backup WTF game assembly to folder",
            ...
            {
                "v|version",
                "Print the WTFModInjector version number",
        ...
        private static bool InjectModHookPoint(ModuleDefinition game, ModuleDefinition injecting)
        {
            ...
            hookedMethod.Body.SimplifyMacros();
            ...
            hookedMethod.Body.OptimizeMacros();
        ...
        {
            SayHeader();
            WriteLine("Usage: WTFModLoaderInjector.exe [OPTIONS]+");
            WriteLine("Inject the WTF game assembly with an entry point for mod enablement.");
        ...
        {
            WriteLine($"Expected WTF game v{expectedVersion}");
            WriteLine($"Actual WTF game v{version}");
        }
        ...
        {
            SayHeader();
            Write("WTFModLoaderInjector.exe: ");
            WriteLine(e.Message);
            WriteLine("Try `WTFModLoaderInjector.exe --help' for more information.");
        }
        ...
        {
            SayHeader();
            WriteLine($"ERROR: We could not find the WTF game assembly {GAME_DLL_FILE_NAME} in directory '{givenManagedDir}'.\n" +
        ...
        {
            SayHeader();
            WriteLine($"ERROR: We could not find the WTF game assembly {MOD_LOADER_DLL_FILE_NAME} at '{expectedModLoaderAssemblyPath}'.\n" +
        ...
        {
            WriteLine("WTFModLoader Injector");
            WriteLine("----------------------------");
        }
        ...
        private static void SayAlreadyInjected(bool isCurrentInjection)
        {
            WriteLine(isCurrentInjection
                ...
                : $"ERROR: {GAME_DLL_FILE_NAME} already injected with an older WTFModLoader Injector.  Please revert the file and re-run injector!");
        }
    ...
    {
        public BackupFileInjected(string backupFileName = "WTF.exe.orig") : base(FormulateMessage(backupFileName))
        {
    ...
    {
        public BackupFileNotFound(string backupFileName = "WTF.exe.orig") : base(FormulateMessage(backupFileName))
        {
            BackupFileName = backupFileName;
        }
